...
<div class="relative flex flex-col items-start w-full gap-xs group" x-data="{ inputValue: '' }">
    <input type="search" id="{{ $id }}" class="rounded-sm bg-black-2 font-raleway font-[400] text-[12px] placeholder-gray-3 border-none text-black-1 dark:text-white focus-visible:outline-0 px-sm h-lg w-full" name="{{ $name }}" x-model="inputValue" placeholder="{{ $placeholder }}">
